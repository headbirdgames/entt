...
protected:
    ...
    void prop(meta_prop_node node) {
        std::vector<meta_prop_node> *prop = nullptr;
        ...
        for(std::size_t pos{}, last = prop->size(); pos != last; ++pos) {
            if((*prop)[pos].id == node.id) {
                (*prop)[pos] = std::move(node);
                return;
            }
        }

        prop->push_back(std::move(node));
    }
